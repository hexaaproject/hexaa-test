...
public class EntitlementsGetTest extends CleanTest {
	...
	public void testEntitlementsGet() {
		...
		JSONObject jsonReaponse = (JSONObject) JSONParser
		...
		try {
			...
			JSONAssert.assertEquals(entitlements.getJSONObject(0),
					jsonReaponse, JSONCompareMode.LENIENT);
		} catch (AssertionError e) {
			AssertErrorHandler(e);
		}

		// GETs the entitlement with the id 2.
		jsonReaponse = (JSONObject) JSONParser.parseJSON(persistent.call(
				Const.Api.ENTITLEMENTS_ID, BasicCall.REST.GET, null, 2, 0));
		try {
			assertEquals(Const.StatusLine.OK, persistent.getStatusLine());
