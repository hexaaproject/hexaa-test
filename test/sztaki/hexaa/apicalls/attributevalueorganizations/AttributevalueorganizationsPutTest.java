...
public class AttributevalueorganizationsPutTest extends CleanTest {
	...
	public static void setUpClass() {
		organizations = Utility.Create
				.organization("AttributevalueorganizationsPutTest_org1");
		if (organizations.length() < 1) {
			fail("Utility.Create.organization( \"AttributevalueorganizationsPutTest_org1\" ); did not succeed");
		}
		attributespecs = Utility.Create.attributespec(
				new String[] { "AttributevalueorganizationsPutTest_as1" },
				"manager");
		if (attributespecs.length() < 1) {
			fail("Utility.Create.attributespec(new String[] {\"AttributevalueorganizationsPutTest_as1\" }, \"manager\"); did not succeed");
		}
		attributevalues = Utility.Create.attributevalueorganization(
				"AttributevalueorganizationsPutTest_org_value1", attributespecs
						.getJSONObject(0).getInt("id"), organizations
						.getJSONObject(0).getInt("id"));
		if (attributevalues.length() < 1) {
			fail("Utility.Create.attributevalueorganization(\"AttributevalueorganizationsPutTest_org_value1\", attributespecs.getJSONObject(0).getInt(\"id\"), organizations.getJSONObject(0).getInt(\"id\")); did not succeed");
		}
	}
	...
	public void testAttributevalueorganizationPut() {
		...
		jsonTemp.put("value",
				"AttributevalueorganizationsPutTest_org_value1_changed");
		jsonTemp.put("attribute_spec",
				attributespecs.getJSONObject(0).getInt("id"));
		jsonTemp.put("organization", organizations.getJSONObject(0)
				.getInt("id"));

		persistent.call(Const.Api.ATTRIBUTEVALUEORGANIZATIONS_ID,
				BasicCall.REST.PUT, jsonTemp.toString(), attributevalues
