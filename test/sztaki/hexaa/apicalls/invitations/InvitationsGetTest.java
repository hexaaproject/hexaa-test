...
public class InvitationsGetTest extends CleanTest {
	...
	public static void setUpClass() {
		...
		invitationOrganization = Utility.Create.invitationToOrg(null,
				"http://hexaa.eduid.hu/hexaaui",
				"This is a test invitation to organization.", 0, 1);

		invitationRole = Utility.Create.invitationToOrg(null,
				"http://hexaa.eduid.hu/hexaaui",
				"This is a test invitation to role.", 1, 1);

		invitationService = Utility.Create.invitationToService(null,
				"http://hexaa.eduid.hu/hexaaui",
				"This is a test invitation to service.", 1);
	}
	...
	public void testInvitationGetOrganization() {
		JSONObject jsonResponse = (JSONObject) JSONParser
				.parseJSON(persistent.call(Const.Api.INVITATIONS_ID,
						BasicCall.REST.GET, null, 1, 1));
		System.out.println(jsonResponse.toString());

		invitationOrganization.put("organization_id",
				invitationOrganization.remove("organization"));

		try {
			assertEquals(Const.StatusLine.OK, persistent.getStatusLine());
			JSONAssert.assertEquals(invitationOrganization, jsonResponse,
	...
	public void testInvitationGetRole() {
		...
		invitationRole.put("organization_id",
				invitationRole.remove("organization"));
		invitationRole.put("role_id", invitationRole.remove("role"));

		try {
			assertEquals(Const.StatusLine.OK, persistent.getStatusLine());
			JSONAssert.assertEquals(invitationRole, jsonResponse,
					JSONCompareMode.LENIENT);
		} catch (AssertionError e) {
			AssertErrorHandler(e);
		}
	...
	public void testInvitationGetService() {
		JSONObject jsonResponse = (JSONObject) JSONParser
				.parseJSON(persistent.call(Const.Api.INVITATIONS_ID,
						BasicCall.REST.GET, null, 3, 3));
		System.out.println(jsonResponse.toString());

		invitationService
				.put("service_id", invitationService.remove("service"));

		try {
			assertEquals(Const.StatusLine.OK, persistent.getStatusLine());
			JSONAssert.assertEquals(invitationService, jsonResponse,
					JSONCompareMode.LENIENT);
		} catch (AssertionError e) {
			AssertErrorHandler(e);
		}
